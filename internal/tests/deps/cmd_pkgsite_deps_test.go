...
var allowedModDeps = map[string]bool{
	...
	"github.com/wow-look-at-my/static-pkgsite":           true,
...
func TestCmdPkgsiteDeps(t *testing.T) {
	...
	out, err := exec.Command("go", "list", "-e", "-deps", "github.com/wow-look-at-my/static-pkgsite/cmd/pkgsite").Output()
	if err != nil {
		if ee, ok := err.(*exec.ExitError); ok && len(ee.Stderr) > 0 {
			t.Fatalf("running go list -test -deps on package github.com/wow-look-at-my/static-pkgsite/cmd/pkgsite:\n%s", ee.Stderr)
		}
		t.Fatalf("running go list -test -deps on package github.com/wow-look-at-my/static-pkgsite/cmd/pkgsite: %v", err)

	}
	pkgs := strings.Fields(string(out))
	for _, pkg := range pkgs {
		// Filter to only the dependencies that are in the pkgsite module.
		if !strings.HasPrefix(pkg, "github.com/wow-look-at-my/static-pkgsite") {
			continue
		}
